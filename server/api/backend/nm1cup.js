...
module.exports = class nm1Cup {
  ...
  async initCollecitons () {
    ...
    this.slimPlayerFields = ['id', 'username', 'avatar_url', 'elo', 'eliminated', 'statistics', 'conver_url', 'last_visit', 'join_date', 'is_active']
  }
  ...
  async getPlayers (tournament) {
    return await this.collections.players.find({ tournament }).toArray().then(arr => arr.map(this.playerSlimFieldsFilter.bind(this)))
  }

  async getPlayer (tournament, id) {
    return await this.collections.players.findOne({ _id: parseInt(id), tournament }).then(p => this.playerSlimFieldsFilter(p))
  }
  ...
  async updateGame (tournament, game) {
    const $unset = {}
    const current = await this.collections.games.findOne({ slug: tournament }) || {}
